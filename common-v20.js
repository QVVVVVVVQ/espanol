...
const REV9_ADDITIONS={verb_practice:['estar alegre']};
...
const PRE_V8_TOPICS=['colors','weekdays','months','ser','conversations','school','adjectives','numerals','tener','animals'];
...
function loadState(){
  ...
  if(currentRevision<5){
    const selected=s.settings.pronunciation.topics||[];
    if(PRE_V5_TOPICS.every(id=>selected.includes(id))&&!selected.includes('tener'))selected.push('tener');
  }
  ...
  if(currentRevision<8){
    const selected=s.settings.pronunciation.topics||[];
    if(PRE_V8_TOPICS.every(id=>selected.includes(id))){
      if(TOPICS.estar&&!selected.includes('estar'))selected.push('estar');
      if(TOPICS.verb_practice&&!selected.includes('verb_practice'))selected.push('verb_practice');
    }
  }
  if(currentRevision<9){
    Object.entries(REV9_ADDITIONS).forEach(([id,words])=>{
      if(!TOPICS[id])return;
      const arr=s.settings.active[id]||(s.settings.active[id]=[]);
      words.forEach(word=>{if(TOPICS[id].words.some(w=>w.es===word)&&!arr.includes(word))arr.push(word)});
    });
